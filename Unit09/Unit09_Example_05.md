...
| 參數 | 符號 | 物理意義 |
|------|------|---------|
| 零剪切黏度 | $\mu_0$ | 低剪切率極限黏度（ $\mathrm{Pa}\cdot\mathrm{s}$ ）|
...
$$
y_1(0) = 0 \quad \text{（管中心對稱）},\qquad y_0(1) = 0 \quad \text{（管壁無滑移）}
$$
...
| 零剪切黏度 | $\mu_0$ | $0.5\;\mathrm{Pa}\cdot\mathrm{s}$ | 高黏度聚合物溶液量級 |
...
![fig02](outputs/Unit09_Example_05/figs/fig02_n_sensitivity.png)

**圖 4.2** 左：不同 $n$ 值的速度剖面（ $n$ 越小，柱塞流越明顯）；右：流量增強因子隨 $n$ 的變化趨勢。

### 4.3 Weissenberg 數 $Wi$ 的影響（圖 fig03）
...
![fig03](outputs/Unit09_Example_05/figs/fig03_Wi_sensitivity.png)
