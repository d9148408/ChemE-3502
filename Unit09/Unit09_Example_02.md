...
| 熱容量（丙酮 A）| $C_{pA}$ | 163 | $\mathrm{J/(mol{\cdot}K)}$ | |
| 熱容量（乙烯酮 B）| $C_{pB}$ | 83 | $\mathrm{J/(mol{\cdot}K)}$ | |
| 熱容量（甲烷 C）| $C_{pC}$ | 71 | $\mathrm{J/(mol{\cdot}K)}$ | |
...
- **溫度下降的原因**：儘管熱交換溫度 $T_a=1250$ K > $T=1035$ K（套管正在加熱），但體積熱傳係數 $U_a=110.6 \; \mathrm{J/(m^3{\cdot}s{\cdot}K)}$ 偏小，熱交換能供給的熱量遠不足以補償吸熱反應所需的能量，導致溫度淨下降
...
- **原因**： $U_a=110.6 \; \mathrm{J/(m^3{\cdot}s{\cdot}K)}$ 遠小於反應所需熱量，熱交換對系統的影響可忽略不計
...
- **熱交換效果微弱的原因**： $U_a=110.6 \; \mathrm{J/(m^3{\cdot}s{\cdot}K)}$ × 反應器體積（1.4×10⁻⁴ m³）所能傳遞的熱量，遠不及吸熱反應所需，顯示此設計尚有優化空間
...
3. 如現有 $U_a$ 提升 100 倍至 $11{,}060 \; \mathrm{J/(m^3{\cdot}s{\cdot}K)}$ ， $T_a$ 的靈敏度會如何改變？
