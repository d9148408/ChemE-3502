...
Stiff 問題的核心困難在於：**系統中同時存在「很快消失的短暫現象」與「緩慢演進的主要行為」**，兩者的時間尺度相差懸殊。

以顯式求解器（如 RK45）為例，步長 $h$ 受最快分量的穩定性限制：

$$
h \leq \frac{C}{|\lambda_{\max}|}
$$

其中 $C$ 為穩定性常數（約 2～3）。若 $|\lambda_{\max}| = 10^6$，即使我們只關心 $t \in [0, 100]$ 的緩慢演進，仍須使用 $h \approx 10^{-6}$ 的微小步長，導致需要 $10^8$ 步計算——這就是 stiff 問題使顯式方法失效的根本原因。

**隱式求解器**（如 Radau、BDF）每步需解一組非線性方程組，計算量較重，但其穩定域幾乎覆蓋整個左半複數平面，可安全使用大步長，僅追蹤感興趣的緩慢動態，大幅降低計算量。

| 求解策略 | 每步計算量 | 步長限制 | 適合情境 |
|----------|-----------|----------|----------|
| 顯式（RK45） | 輕 | 受最快特徵值限制（嚴格） | Non-stiff 問題 |
| 隱式（Radau/BDF） | 重（需解方程組） | 幾乎不受限（A-stable） | Stiff 問題 |

### 3.2 化工典型 Stiff 問題

化工領域中，stiff ODE 幾乎無所不在。以下分類說明各類系統的 stiffness 來源：

| 系統類別 | 代表系統 | Stiff 來源 | 典型 SR 量級 |
|----------|---------|-----------|-------------|
| 化學反應動力學 | 燃燒、爆炸、自由基鏈反應 | 快速自由基步驟 $k \sim 10^{10}$ vs 慢速整體轉化 | $10^6$～$10^{12}$ |
| 生化反應 | 酵素動力學、發酵、代謝路徑 | Michaelis-Menten 快速平衡假設失效時 | $10^3$～$10^6$ |
| 化學反應器 | 串聯 CSTR、批式反應器 | 放熱反應溫度靈敏性造成時間常數差異 | $10^3$～$10^5$ |
| 質傳耦合反應 | 氣液吸收反應、觸媒反應 | 擴散時間常數遠小於反應時間常數 | $10^4$～$10^8$ |
| 熱傳導 | 換熱器、反應器溫度控制 | 局部熱點快速響應 vs 整體溫度場緩慢調整 | $10^2$～$10^4$ |
| 電化學 | 電池模型、電解槽 | 電雙層電容快速充電 vs 擴散控制緩慢響應 | $10^4$～$10^8$ |

#### 重點系統詳細說明

**1. 化學反應動力學（燃燒 / 自由基反應）**

燃燒及爆炸反應涉及數十至數百個基元反應步驟，其中自由基（如 $\mathrm{OH \cdot}$、$\mathrm{H \cdot}$、$\mathrm{O \cdot}$）的反應速率常數可達 $k \sim 10^{10}\ \mathrm{L/(mol \cdot s)}$，而整體燃料轉化則發生在毫秒至秒的尺度。Robertson 問題是化工教科書最常引用的 stiff 範例：

$$
\frac{dy_1}{dt} = -0.04\,y_1 + 10^4\,y_2 y_3, \quad
\frac{dy_2}{dt} = 0.04\,y_1 - 10^4\,y_2 y_3 - 3\times10^7\,y_2^2, \quad
\frac{dy_3}{dt} = 3\times10^7\,y_2^2
$$

三個速率常數分別為 $0.04$、$10^4$、$3 \times 10^7$，相差約 $10^9$ 倍，SR 極大。

**2. 生化反應（酵素動力學 / 代謝路徑）**

Michaelis-Menten 動力學在「快速平衡假設」不成立時，必須以全方程組建模：

$$
\frac{d[\mathrm{E}]}{dt} = -k_1[\mathrm{E}][\mathrm{S}] + (k_{-1}+k_2)[\mathrm{ES}], \quad
\frac{d[\mathrm{ES}]}{dt} = k_1[\mathrm{E}][\mathrm{S}] - (k_{-1}+k_2)[\mathrm{ES}]
$$

酵素-基質複合物 $[\mathrm{ES}]$ 的形成/解離（毫秒）遠快於產物生成（秒至分鐘），造成強烈 stiffness。代謝路徑模型（如糖解、TCA cycle）同時含數十個酵素步驟，SR 可達 $10^6$ 以上。

**3. CSTR 與批式反應器（放熱反應）**

帶有 Arrhenius 反應速率的 CSTR，當熱效應顯著時，溫度-濃度耦合會產生多重穩定態。在穩定稳态附近進行線性化時，Jacobian 特徵值可出現實部差異達 $10^3$ 倍的情況，為典型化工 stiff 系統。含自我催化（autocatalytic）或連串反應的批式反應器亦屬此類。

**4. 質傳-反應耦合系統**

氣體吸收伴隨化學反應（如 $\mathrm{CO_2}$ 被 $\mathrm{MEA}$ 溶液吸收）中，氣液界面濃度在微秒尺度達到平衡，而整體吸收塔的傳質則需數秒至數分鐘。若以節點法或有限差分法對空間離散化，所得半離散 ODE 系統通常為強 stiff 問題，SR 隨節點數增加而惡化（空間步長縮小，SR $\propto 1/\Delta x^2$）。

**5. 電化學系統**

鋰離子電池 (Li-ion battery) 的 P2D（Pseudo-2D）模型同時描述電雙層（時間尺度 $\sim \mu\mathrm{s}$）、固相擴散（$\sim \mathrm{s}$）與副反應（$\sim \mathrm{h}$），跨越 $10^7$ 倍時間尺度差異，是近年化工領域最受關注的極端 stiff 問題之一。
...
- 化工中類似情形包括含自由基的燃燒動力學（快速自由基 $\tau \sim \mu\mathrm{s}$ ，全局轉化 $\tau \sim \mathrm{ms}$ ）與質傳-反應耦合系統。
...
- **RK45**：需要 8,120,324 次函數呼叫、耗時 124.3 秒，為強 stiff 問題下的典型失效模式——雖最終成功求解，但效率極低。
- **Radau**：僅需 2,609 次呼叫、耗時 0.107 秒，較 RK45 快約 **1162 倍**；精度高（Radau IIA 5 階），為化工 stiff 問題的**首選求解器**。
- **BDF**：1,589 次呼叫、0.117 秒；BDF（逆向微分公式）是大型 stiff 系統的標準選擇，此案例與 Radau 效能相當。
...
- **相圖（右圖）**： $C_A$-$C_B$ 相空間軌跡從初始點（綠圓，右下角）沿弧形單調收斂至平衡點（紅星，左上角）；途中 $C_B$ 先升後降形成峰值，顯示系統具唯一穩態。
