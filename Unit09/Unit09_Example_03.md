...
本系統的關鍵特徵是：**產物降解速率常數 $k_p = 500\ \mathrm{h^{-1}}$ 遠大於細胞最大比生長速率 $\mu_{max} = 0.4\ \mathrm{h^{-1}}$**，
...
產物合成（ $q_p X$ ）與降解（ $k_p P$ ）的淨效應； $k_p = 500\ \mathrm{h^{-1}}$ 遠大於生長速率，是**剛性來源**
...
$$
\mathbf{J} = \frac{\partial \mathbf{f}}{\partial \mathbf{y}} = \begin{bmatrix} \mu - k_d & \dfrac{\partial \mu}{\partial S} X & 0 \\ -\dfrac{\mu}{Y_{xs}} & -\dfrac{\partial \mu}{\partial S} \dfrac{X}{Y_{xs}} & 0 \\ q_p & 0 & -k_p \end{bmatrix}
$$
...
- Jacobian 的 $J[2,2] = -k_p = -500\ \mathrm{h^{-1}}$ ，遠大於 $J[0,0] = \mu - k_d \approx 0.38\ \mathrm{h^{-1}}$
...
- 三個時間點的特徵值分佈均顯示：在 $\lambda \approx -500$ 處有一個快速特徵值（ $\lambda_3 = -k_p = -500\ \mathrm{h^{-1}}$ ），
  對應產物降解的快速動力學
...
- **基質 $S(t)$** （中圖）：從 5.0 g/L 起始，在 $t \approx 9$ h 速度加快耗盡（指數生長期結束），
  之後趨近於 0；出口 $S(24) \approx 0$ g/L（基質完全耗盡；隱式求解器之微小數值偏差已截斷為零）
...
| 剛性來源 | 一個快速動力學主導 $J$ 最大特徵值 | $k_p = 500\ \mathrm{h^{-1}}$ → $\lambda_3 = -500$ |
...
|:---:|:---:|
| 產物降解 $k_p = 500\ \mathrm{h^{-1}}$ → $\tau_{fast} = 1/500 = 0.002$ h | 細胞生長 $\mu_{max} = 0.4\ \mathrm{h^{-1}}$ → $\tau_{slow} = 1/0.4 = 2.5$ h |
| 快速反應平衡 | 緩慢傳質 |
| 短半衰期組分 | 長半衰期組分 |
...
本例中 $k_p = 500\ \mathrm{h^{-1}}$ 遠大於 $\mu_{max} = 0.4\ \mathrm{h^{-1}}$ ，QSS 假設準確度高——圖示結果確認 $P(t)$ 與 $P^*(t)$ 近乎完全重合（誤差 $< 10^{-3}$ g/L）。
