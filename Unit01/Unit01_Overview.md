...
```
SciPy 在 NumPy 基礎上提供線性代數、最佳化、ODE 求解、插值、統計等高階功能，是本課程 Unit05–Unit15 的主要計算工具。
...
對一組批次反應實驗數據，透過 `scipy.optimize.curve_fit()` 將動力學模型與實驗量測值進行非線性最小平方擬合，估計反應速率常數與活化能，並計算各參數的 95% 信賴區間，是化工數據分析中最常見的建模流程。
...
**Python vs MATLAB ：**
> MATLAB 在控制系統和動態模擬方面的工具箱相當完整。然而，對於資料科學、機器學習，以及開源協作場景，Python 有著更廣泛的生態系統支援和零授權成本的優勢。本課程選擇 Python 作為主要工具，是考量到其開源免費、跨領域通用性、以及在產業界（特別是 AI/ML 領域）快速增長的應用比例。
...
| 單元 | 主題 | 核心工具 |
|------|------|----------|
| Unit05 | SciPy 概述與特殊函數 | 各子模組功能導覽、`scipy.special` 化工相關特殊函數應用 |
...
#### 模組三：參數估計、統計分析、信號處理與機器學習 (Unit13–Unit18)

| 單元 | 主題 | 核心工具 |
|------|------|----------|
| Unit13 | 參數估計 | `scipy.optimize.curve_fit()`, 最小平方法 |
| Unit14 | 統計分析 | `scipy.stats`, 機率分布、假設檢定、描述統計 |
| Unit15 | 信號模擬與處理 | `scipy.signal`, 濾波、系統建模、頻率分析 |
| Unit16 | 基礎 AI－線性回歸 | `sklearn.linear_model` |
| Unit17 | 基礎 AI－非線性回歸 | `sklearn.preprocessing.PolynomialFeatures` + `sklearn.linear_model` |
| Unit18 | 基礎 AI－神經網路 | `sklearn.neural_network.MLPRegressor` |
...
```
           ...
           └─────────────────────┬───────────────────────┘
                                 ...
                                 ▼
           ┌─────────────────────────────────────────────┐
           │    模組三：參數估計、統計分析、信號處理與機器學習 │
           │  Unit13 (參數估計) → Unit14 (統計分析)        │
           │    → Unit15 (信號處理) → Unit16 (線性回歸)    │
           │    → Unit17 (非線性回歸) → Unit18 (神經網路)  │
           └─────────────────────────────────────────────┘
```
...
```
特殊函數 (Unit05)  →  Bessel/Gamma/誤差函數等特殊函數應用於傳輸現象、反應工程、熱力學計算
線性方程 (Unit06)  →  物料/能量平衡、混合問題、蒸餾塔成分計算
非線性方程 (Unit07) →  狀態方程式求解、反應器多重穩態、泡露點計算
插值積分 (Unit08) →  實驗數據插值、反應熱計算、數值積分求解
ODE (Unit09)       →  批次反應器動態、CSTR動態響應、PFR轉化率分布
PDE (Unit10)       →  非穩態熱傳、擴散反應、管流速度場
FFT (Unit11)       →  製程訊號頻譜分析、振動模態識別、週期性干擾偵測
最適化 (Unit12)    →  製程條件最佳化、能量整合、操作成本最小化
```

**模組三（參數估計、統計分析、信號處理與機器學習）與化工問題的對應：**

```
參數估計 (Unit13)     →  動力學常數估計、傳遞係數擬合、QSPR 模型
統計分析 (Unit14)     →  製程品質管制、實驗數據假設檢定、分布分析
信號處理 (Unit15)     →  製程噪聲濾波、感測器信號分析、頻率響應
線性回歸 (Unit16)     →  製程性能預測、品質線性模型、製程變數相關性
非線性回歸 (Unit17)   →  軟感測器開發、多項式製程模型
神經網路 (Unit18)     →  複雜非線性系統建模、MLP 製程預測
```
