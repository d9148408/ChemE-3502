...
**結果說明**：
- 程式碼偵測到符號變化於 $[0.3639,\ 0.3643]$ L/mol，隨後以 $\pm 5 \times 10^{-5}$ m³/mol 安全邊際擴大為實際計算區間 $[0.3139,\ 0.4143]$ L/mol（寬度 $1.004 \times 10^{-4}$ m³/mol），確保區間有效
- 經 **28 次迭代**收斂至 $V = 0.364142$ L/mol；收斂速率為線性，每次迭代區間縮小一半，理論上需 $\lceil \log_2(1.004 \times 10^{-1} / 10^{-12}) \rceil = 37$ 次，實際 28 次提前收斂（`|b-a| < tol = 1e-12` 條件先達到）
...
- 起始值 $x_0 = 3 \times 10^{-4}$ m³/mol（使用理想氣體估算）已足夠接近真實根，因此迭代未發散
