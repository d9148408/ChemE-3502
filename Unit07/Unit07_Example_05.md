...
| 符號 | 說明 | 值 | 單位 |
|------|------|-----|------|
| $h$ | 對流熱傳係數 | 60 | $\mathrm{W/(m^2 \cdot K)}$ |
| $T_{air}$ | 乾燥空氣溫度 | 80 | °C |
| $T_s$ | 乾燥面溫度（**待求**） | — | °C |
| $\lambda$ | 水的蒸發潛熱 | 2.4×10⁶ | J/kg |
| $k_y$ | 質傳係數 | 0.04 | $\mathrm{kg\,dry\,air/(m^2 \cdot s)}$ |
...
| $N_A = 3.845\ \mathrm{kg/(m^2 \cdot h)}$ | 乾燥速率 | 每平方米乾燥面每小時移除的水分量 |
...
固定 $\phi_{air} = 5\%$ 、 $h = 60\ \mathrm{W/(m^2 \cdot K)}$ ，掃描 $T_{air}$ 從 50°C 到 130°C：
...
固定 $T_{air} = 80°C$ 、 $h = 60\ \mathrm{W/(m^2 \cdot K)}$ ，掃描 $\phi_{air}$ 從 0% 到 60%：
...
|----------------|---------------|-------------|
| 5% | $3.845\ \mathrm{kg/(m^2 \cdot h)}$ | 100%（基準） |
| 10% | $3.428\ \mathrm{kg/(m^2 \cdot h)}$ | 89.1% |
| 30% | $2.210\ \mathrm{kg/(m^2 \cdot h)}$ | 57.5% |
| 50% | $1.382\ \mathrm{kg/(m^2 \cdot h)}$ | 35.9% |
...
| **恆率乾燥速率** | $N_A$ | **$3.8452\ \mathrm{kg/(m^2 \cdot h)}$** |
