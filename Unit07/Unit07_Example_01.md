...
- $V$：莫耳體積 (m³/mol)
- $T$：溫度 (K)
- $R = 8.314\ \mathrm{J/(mol \cdot K)}$：氣體常數
- $a$：分子間引力修正參數 ($\mathrm{Pa \cdot m^6/mol^2}$)
- $b$：分子體積修正參數 (m³/mol)
...
| 性質 | 符號 | 數值 | 單位 |
|------|------|------|------|
| VdW 引力參數 | $a$ | 0.3658 | $\mathrm{Pa \cdot m^6/mol^2}$ |
...
| 根 | 大小 | 物理意義 |
|----|------|---------|
| 最小根 $V_L$ | $b$ 至數倍 $b$ | **液相**莫耳體積 |
| 中間根 $V_M$ | 介於 $V_L$ 和 $V_G$ 之間 | 數學根，**無物理意義**（$(\partial P/\partial V)_T > 0$，熱力學不穩定） |
...
2. **newton（二次收斂，需導數）**：6 次迭代，需額外計算 $\partial f/\partial V$
...
對於氮氣（N₂）（$a = 0.1370\ \mathrm{Pa \cdot m^6/mol^2}$，$b = 3.87 \times 10^{-5}$ m³/mol），由於分子間作用力較弱（$a$ 值較小），在相同條件下偏離理想氣體的程度小於 CO₂。
...
1. **CO₂（非極性但具大四極矩）**：
   ...
   - 分子間引力最強（ $a = 0.3658\ \mathrm{Pa \cdot m^6/mol^2}$），使實際體積顯著小於理想預測
   - CO₂ 雖為線型對稱非極性分子（偶極矩 $\mu = 0$），但具有大四極矩與較大分子尺寸，導致 $a$ 值最大
   - 對應 $T_r = T/T_c = 0.986$（接近臨界溫度），分子間作用力效應放大

2. **N₂（非極性小分子）**：
   - $Z = 0.9825$，僅偏離 **-1.75%**
   - 分子間引力最弱（ $a = 0.1370\ \mathrm{Pa \cdot m^6/mol^2}$），接近理想氣體行為
