...
總流量 = 100.0000 kg/hr (應為 100)
殘差 ||Ax - b|| = 0.00e+00
矩陣條件數 κ(A) = 1.7910

產品組成驗證:
  成分 A: 計算=40.00, 目標=40.00
  成分 B: 計算=35.00, 目標=35.00
  成分 C: 計算=25.00, 目標=25.00
```

> **分析**：係數矩陣條件數 $\kappa(A) = 1.79$，屬於良態系統，求解精確（殘差機器精度）。三種原料各需 38.98、38.42、22.60 kg/hr，合計恰好 100 kg/hr，且各成分組成完全符合目標規格 A=40%、B=35%、C=25%。
...
# 問題設定：HX1（間接熱交換器）+ 直接混合點 2 + 下游冷流混合點
# 未知數: T1_out (熱流中間溫度), T2_out (冷流 A 出口), T3_out (混合點 2 出口溫度), 單位 °C
# 設備參數 — mCp (kW/°C): 熱流 2.0, 冷流 A 1.5, 冷流 B 3.0
# 入口溫度: 熱流 200°C, 冷流 A 30°C, 冷流 B 25°C; 混合產品目標: 90°C

# 方程式建立
# 熱交換器 1 (間接): 2.0*(200 - T1_out) = 1.5*(T2_out - 30)
#   → 2.0*T1_out + 1.5*T2_out = 2.0*200 + 1.5*30 = 445
# 直接混合點 2 (熱流與冷流 B 直接混合，T3_out 為混合出口溫度):
#   2.0*T1_out + 3.0*25 = (2.0+3.0)*T3_out
#   → 2.0*T1_out - 5.0*T3_out = -75
# 下游混合目標: 冷流 A 與混合點 2 出口在下游混合點需達產品溫度 T_product = 90°C
...
print(f"\n熱交換器 1 熱負荷: {Q1:.2f} kW")
print(f"直接混合點 2 熱轉移量: {Q2:.2f} kW")

# 下游混合驗證
T_product = (1.5 * T_out[1] + 3.0 * T_out[2]) / (1.5 + 3.0)
print(f"下游混合產品溫度驗證: {T_product:.2f} °C (應為 90 °C)")
```
...
熱交換器網絡出口溫度:
  T1_out = 106.25 °C  (熱流中間溫度)
  T2_out = 155.00 °C  (冷流 A 出口)
  T3_out =  57.50 °C  (直接混合點 2 出口)

熱交換器 1 熱負荷: 187.50 kW
直接混合點 2 熱轉移量:  97.50 kW
下游混合產品溫度驗證: 90.00 °C (應為 90 °C)
```

> **分析**：HX1 將熱流從 200°C 冷卻至 106.25°C，釋放 187.5 kW 用於加熱冷流 A（30°C → 155°C）。直接混合點 2 使熱流（106.25°C）與冷流 B（25°C）混合，混合出口溫度為 57.5°C（相當於 97.5 kW 熱量由熱流傳至冷流 B）。下游冷流混合點將冷流 A（155°C, mCp=1.5）與混合出口（57.5°C, mCp=3.0）混合，驗算 $(1.5 \times 155 + 3.0 \times 57.5)/4.5 = 90.0$ °C ✓，完全符合產品目標。
