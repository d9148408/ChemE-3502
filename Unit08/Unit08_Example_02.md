...
本程式於 PY310（Python 3.10）環境執行，套件版本如下：
...
> **說明**：訓練數據形成 6（溫度）× 5（濃度）= 30 個網格節點，$D$ 值域為 0.59～4.78 × 10⁻⁹ m²/s。測試點 P1～P3 均位於訓練網格內部（非外插），可有效評估插值精確度。
...
- **右**：RBS-Cubic（三次 B-spline）— 等高線與中圖高度一致，細部略有差異

**定量細節：**

- 色彩尺度範圍：0.4 到 4.95 × 10⁻⁹ m²/s，共 20 個層級
- 訓練點：30 個黑色圓點（6 溫度 × 5 濃度）
- 測試點：3 個綠色三角形，均位於訓練網格內部

三種方法的等高線形態整體相似，均呈現「右下角高值（高溫低濃度）→ 左上角低值（低溫高濃度）」的梯度趨勢，符合物理直觀。在雙線性圖中可輕微見到網格點附近的折角線條，而三次插值方法則產生光滑的曲線。
...
- **右**：差值曲面 $\Delta D = D_{\text{RBS}} - D_{\text{RGI-Linear}}$ ，差值皆為負或零（RBS 略低於雙線性），最大絕對差值約 0.02 × 10⁻⁹ m²/s（對比最小 D 值 0.59 尚不到 3.5%），表明兩者在網格內部的預測差異微小

**物理詮釋：**

曲面猶如一座傾斜山丘：從低溫-高濃度方向（左後低谷）向高溫-低濃度方向（右前高峰）攀升，最高點（T=50°C、C=10wt%）的 $D$ 值達 4.78，為最低點（T=25°C、C=50wt%）的 8.1 倍。左圖與中圖的色彩分布高度吻合，右側差值曲面的值域為 −0.02 至 0 × 10⁻⁹ m²/s（差值均為負或零，即 RBS 略低於雙線性）。
...
- **左**：各測試點的相對誤差柱狀圖。P3（47°C, 45wt%）誤差最大，因其在溫度軸與濃度軸上均靠近網格邊界（近邊界插值效應），且兩軸均為偏角落的位置
