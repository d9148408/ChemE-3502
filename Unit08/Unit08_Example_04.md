...
對氣固兩相流提升管的微元段 $dz$ 進行**垂直方向動量平衡**，忽略摩擦力（friction）及加速度項後，簡化為：
...
手動計算前向差分、後向差分、中間差分，與 `numpy.gradient()` 結果對照：
...
中間差分與 numpy.gradient() 結果比較（最大絕對差）:
  max|Δ(dP/dz)| = 0.097619 kPa/m
```

**差異分析：**

| $z$ (m) | 前向差分 | 後向差分 | 中間差分 | `numpy.gradient()` | 差值 (中間 vs grad) |
|:-------:|:--------:|:--------:|:--------:|:------------------:|:-------------------:|
| 0.0 | −2.733 | −2.733 | −2.733 | −2.733 | 0.000 |
| 0.3 | −2.050 | −2.733 | −2.343 | −2.440 | **0.097** |
| 0.7 | −1.720 | −2.050 | −1.867 | −1.903 | 0.036 |
| 1.2 | −1.133 | −1.720 | −1.400 | −1.453 | 0.053 |
| 1.8 | −0.957 | −1.133 | −1.038 | −1.052 | 0.014 |
| 2.5 | −0.700 | −0.957 | −0.829 | −0.829 | 0.000 |
| 3.2 | −0.557 | −0.700 | −0.629 | −0.629 | 0.000 |
| 3.9 | −0.529 | −0.557 | −0.543 | −0.543 | 0.000 |
| 4.6 | −0.525 | −0.529 | −0.527 | −0.526 | 0.001 |
| 5.0 | −0.525 | −0.525 | −0.525 | −0.525 | 0.000 |

> 最大差異出現在 $z = 0.3\ \mathrm{m}$ 處（約 0.098 kPa/m），此處相鄰間距差異最大（ $\Delta z_l = 0.3\ \mathrm{m}$ ， $\Delta z_r = 0.4\ \mathrm{m}$ ），充分展示 `numpy.gradient()` 加權公式對非均勻間距的修正效果。均勻間距區段（如 $z = 2.5\text{–}3.9\ \mathrm{m}$ 之間距均為 0.7 m）兩種方法結果完全一致。
