...
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 黏度 $\mu$ (mPa$\cdot$s) | 1.792 | 1.307 | 1.002 | 0.653 | 0.467 | 0.355 | 0.282 |

**測試數據（預留用於精確度驗證）**：

| 溫度 $T$ (°C) | 30 | 50 | 70 |
|:---:|:---:|:---:|:---:|
| 文獻 $\mu$ (mPa$\cdot$s) | 0.798 | 0.547 | 0.404 |
...
| `'cubic'` | 三次樣條插值 | $C^2$ 連續三次多項式插值，平滑度與 CubicSpline 相近 |
...
`CubicSpline` 建立一個**全段 $C^2$ 連續**的三次樣條插值，格式如下：
...
**左圖**：四種方法在 $T = 0 \sim 100$ °C 範圍的完整插值曲線比較。
Nearest（藍色虛線）呈現明顯階梯狀；
Linear（綠色點劃線）為分段折線；
Cubic（紅色實線）與 CubicSpline（品紅色實線）曲線幾乎重疊，
整體輪廓最貼近物理上的平滑衰減趨勢。
橘色三角形為測試數據點（文獻值），黑色實心圓為訓練數據點。

**右圖**：僅顯示 Linear 與 CubicSpline 兩種方法的精細比較。
以**綠色雙向箭頭**標示各測試點（ $T = 30, 50, 70$ °C ）處 Linear 插值值與文獻真值之誤差，
可見 CubicSpline 曲線幾乎完全通過文獻值（橘色三角形），
Linear 在 $T = 20 \sim 40$ °C 大間距區段明顯高估黏度值。
...
> **已知目標黏度 $\mu^* = 0.500$ mPa$\cdot$s，求對應的操作溫度 $T^*$**
...
# 定義目標方程式 g_inverse(T) = f_cs(T) - mu_target = 0
def g_inverse(T):
...
**解讀**：
純水在 $T^* \approx 55.6$ °C 時，動力黏度約為 0.500 mPa$\cdot$s。
...
橘色水平虛線為目標黏度值 $\mu^* = 0.500$ mPa$\cdot$s，
品紅色曲線為 CubicSpline 插值函數，
兩線交叉點即為逆向插值解（紅色實心圓），
紅色垂直點線標示對應溫度 $T^* = 55.56$ °C，
並於圖中以深紅色文字（含箭頭指向交叉點）分兩行標注 $T^* = 55.56$ °C 與 $\mu^* = 0.5$ mPa$\cdot$s，確認計算結果。
...
**左圖**：在三個測試溫度點（ $T = 30, 50, 70$ °C ）的各方法相對誤差（%）比較。
Nearest（藍色棒）誤差最高，在 $T = 30$ °C 達 25.56%；
Linear（橘色棒）介於 1.73% ~ 3.70%；
Cubic 與 CubicSpline（綠/紅色棒）誤差極小（ ≤ 0.18% ），棒圖幾乎不可見。

**右圖**：各方法 MARE 彙整比較（棒柱頂端標注精確數值）。
清楚呈現三次樣條插值在黏度此類非線性物性數據上的顯著優勢：
MARE 由 Nearest 的 20.18% → Linear 的 2.60% → Cubic/CubicSpline 的 0.10%，
精確度較 Nearest 提升約 200 倍、較 Linear 提升約 26 倍。
...
指數關係造成黏度曲線**上凸（concave up， $d^2\mu/dT^2 > 0$ ）**，
在大間距資料區段（如 $T = 20 \to 40$ °C，跨距 20°C）中，
連接兩端點的直線（弦）位於曲線上方，線性插值因此高估了 $\mu$ 值。
三次插值能自動利用鄰近點的斜率資訊，估計曲率方向，大幅改善精確度。

**Cubic 與 CubicSpline 差異為何不明顯？**

本案例數據特性為單調遞減且曲率適中，
`interp1d(kind='cubic')` 與 `CubicSpline` 均為 $C^2$ 連續的三次樣條插值，在本案例中結果幾乎相同。
若數據有局部極値或劇烈振盪，兩者結果差異會更顯著（建議使用 `CubicSpline` 以獲得更多控制選項）。
...
| `CubicSpline` | $C^2$ 全域平滑，not-a-knot 邊界條件為預設，精確度優於線性 |
