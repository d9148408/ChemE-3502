...
其中：
- $\mathbf{J}_A$ \[mol/(m² $\cdot$ s)\]：組分 A 的摩爾通量向量
...
- **$x = 0$**（介質表面）：突然施加固定濃度 $C_{A0}$，Dirichlet 邊界條件
- **$x = L$**（計算域截斷）：Neumann 零通量，模擬「無限遠無擾動」
- 解析解呈 $\mathrm{erfc}(x/2\sqrt{D_{AB}t})$ 形狀，擴散前緣正比於 $\sqrt{D_{AB}\,t}$
...
```
  ...
  C_A0     ←擴散     │對稱軸│     擴散→      C_A0
   ...
   │  初始：C_A,i    │      │   初始：C_A,i    │
  x=-L              x=0                     x=+L
  Dirichlet      Neumann（對稱）            Dirichlet
  C_A = C_A0    ∂C_A/∂x = 0              C_A = C_A0
...
- **$x = \pm L$**（兩端面）：Dirichlet $C_{A0}$，濃度由兩端邊緣向中心趨近
- **$x = 0$**（中心對稱軸）：Neumann 零通量，雙面對稱使中心無淨通量
- Fourier 數 $Fo = D_{AB}t/L^2$ 描述平板趨近均勻狀態的速率
...
**場景一**（半無限域）使用 erfc，**場景二**（有限域）改用 Fourier 級數——根本原因是**計算域是否有限**：

| 條件 | 場景一（半無限） | 場景二（有限平板） |
|------|---------------|-----------------|
| 域 | $x \in [0, \infty)$ | $x \in [0, L]$（有限） |
| 解法 | Laplace 變換 → erfc | 分離變數 → Fourier 級數 |
| 解的形式 | 連續積分（erfc） | 離散疊加（Σ 求和） |

**Fourier 級數的選用邏輯（分離變數法）：**

**第一步**：引入無因次濃度，使邊界條件齊次化：

$$
\theta = \frac{C_A - C_{A0}}{C_{A,i} - C_{A0}}
$$

轉換後邊界條件變為 $\theta(L, t) = 0$，方程形式不變（ $\partial\theta/\partial t = D_{AB}\,\partial^2\theta/\partial x^2$ ）。

**第二步**：假設 $\theta(x, t) = X(x)\,T(t)$（分離變數），代入後得：

$$
\frac{T'}{D_{AB}\,T} = \frac{X''}{X} = -\lambda^2 \quad \Rightarrow \quad X'' + \lambda^2 X = 0
$$

**第三步**：有限域 + 邊界條件（ $X'(0) = 0$，$X(L) = 0$）構成 **Sturm-Liouville 特徵值問題**，其本徵函數為：

$$
X_n(x) = \cos\!\left(\frac{(2n+1)\pi x}{2L}\right), \quad n = 0, 1, 2, \ldots
$$

這些 cos 函數在 $[0, L]$ 上彼此**正交**，因此任意初始條件 $\theta(x, 0)$ 均可展開為它們的線性疊加，構成完備的 Fourier 余弦級數。

> **與場景一的本質差異**：半無限域沒有第二個邊界，特徵值是連續的（ $\lambda \in \mathbb{R}$ ），「求和」退化為積分，積分後的閉合式正好是 erfc。有限域因兩端邊界約束，特徵值離散，解自然呈 Fourier 級數形式。
...
- **$\phi \ll 1$**：擴散遠快於反應，整個觸媒層均能充分利用（ $\eta \approx 1$ ）
- **$\phi \gg 1$**：反應快，組分 A 在表面附近即消耗殆盡，中心無法利用（ $\eta \approx 1/\phi$ ）
- Thiele 模數 $\phi = L\sqrt{k_1/D_{AB}}$ 是設計觸媒顆粒尺寸的關鍵無因次群
...
```
         ...
         │                               │
Dirich.  │       矩形薄板                │  Dirich.
C_A = 1  │     C_A(x, y, t)             │  C_A = 0
(x = 0)  │    Lx × Ly                   │  (x = Lx)
         │                               │
         └───────────────────────────────┘  y = 0
                    Neumann: ∂C_A/∂y = 0
         ← 高濃度從左邊（x=0）向右滲透（主梯度方向）→
```

- **左邊 $x=0$**：Dirichlet $C_A = 1$（高濃度驅動面）
- **右邊 $x=L_x$**：Dirichlet $C_A = 0$（低濃度出射面）
- **下邊 $y=0$**：Neumann 零通量（對稱面或絕質壁）
- **上邊 $y=L_y$**：Dirichlet $C_A = 0$（上邊界吸收）
- 2D 效應使等濃度線在 $y$ 方向彎曲，無法用兩個獨立的 1D 問題疊加求解
...
- **$r = 0$**（軸心）：軸對稱條件，`py-pde` 自動處理（無需手動設定）
- **$r = R$**（圓柱外壁）：Dirichlet $C_{A0}$，由外壁向軸心徑向滲透
- **$z = \pm H$**（上下端面）：Dirichlet $C_{A0}$，端面滲透加速整體飽和
- 同時來自外壁（ $r$ 方向）與端面（ $z$ 方向）的雙向滲透，使圓柱幾何比等截面平板更快達到飽和
...
```
  ...
  邊界條件（以三面 Dirichlet + 三面 Neumann 為例）：
  ┌─────────────────┬───────────────┬──────────────────────────┐
  │ 面               │ 條件          │ 物理意義                  │
  ├─────────────────┼───────────────┼──────────────────────────┤
  │ x=0, y=0, z=0   │ Dirichlet     │ 高濃度驅動面              │
  │ （三個低 index）  │ C_A = C_A0   │ 角落最先達到高濃度        │
  ├─────────────────┼───────────────┼──────────────────────────┤
  │ x=L, y=L, z=L   │ Neumann       │ 截斷邊界（無通量）        │
  │ （三個高 index）  │ ∂C_A/∂n = 0  │ 遠場近似                  │
  └─────────────────┴───────────────┴──────────────────────────┘
```

- 高濃度從三個低 index 角面同時向對角（ $x=y=z=L$ 方向）滲透，中心區域最後飽和
- 3D 擴散不可拆分為三個獨立的 1D 問題（除非各方向邊界條件完全解耦）
- 視覺化建議：取 $z=L/2$ 中心截面的 2D 等高線圖，觀察中心平面的濃度分布
...
- **$r = 0$**（球心）：Neumann $\partial C_A/\partial r = 0$，`py-pde` `SphericalGrid` 自動處理奇異點
- **$r = R$**（球面）：Dirichlet $C_{A0}$，模擬「突然暴露於高濃度環境」
- 球形幾何的外表面積與體積之比為 $3/R$，比等尺寸平板（ $1/L$ ）或圓柱（ $2/R$ ）更大，因此飽和速率最快
- `SphericalGrid` 僅需求解 1D 徑向問題，大幅降低球體 3D 計算的成本
