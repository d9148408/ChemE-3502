...
一塊無限大的平板原本靜止，在 $t = 0$ 時瞬間以速度 $U_0$ 沿板面方向（ $x$ 方向）加速。求板面附近流體的速度場 $u(y, t)$ 隨時間與距離的演變。
...
```
          ...
          ↑
          │        流體（靜止或受板面拖曳）
  y=0.1 ──┤─────────────────────────────────  ← 數值截斷邊界 u(L,t) ≈ 0
          │  u(y,t) ← 速度剖面（只有 x 分量）
          │  ────→
          │   ──→
  y = δ(t)┤    →                              ← 邊界層前緣 δ(t)=4√νt
          │    →
          │    →
  y = 0 ──┤════════════════════════════════   ← 平板（無限大，只看截面）
          │     ══════════════════════════ →  U₀ （t > 0 瞬間起動）
...
- **求解域（1D）**： $y \in [0, L]$ ，其中 $L$ 為截斷高度（足夠大，使遠場 $u \approx 0$ ）
- **平板位於 $y = 0$**：沿 $x$ 方向無限延伸，在 $t < 0$ 靜止；在 $t \ge 0$ 以恆速 $U_0$ 向右移動
- **流體佔據 $y > 0$ 的半無限空間**：初始完全靜止，板面運動透過黏滯力逐漸將動量「擴散」至遠場
- **邊界層厚度 $\delta(t)$**：定義為速度衰減至 $U_0$ 的 1% 的距離，隨時間以 $\sqrt{t}$ 增長
- **為何只有 $y$ 方向**：因平板無限大且均勻移動，流場在 $x$ 和 $z$ 方向完全均勻（ $\partial/\partial x = \partial/\partial z = 0$ ），問題退化為純 1D
...
> 此方程式在數學形式上與熱傳方程式 $\partial T/\partial t = \alpha \partial^2 T/\partial y^2$ 完全相同，差別僅在於「擴散的物理量」從溫度換成了 $x$ 方向動量 $\rho u$，擴散係數從熱擴散率 $\alpha$ 換成了運動黏度 $\nu$。
...
| 條件 | 幾何位置 | 物理意義 | BC 類型 |
|------|---------|---------|---------|
| $u(0,t) = U_0$ | 平板表面 $y=0$ | 流體黏附於板面，與板速相同（no-slip） | Dirichlet |
| $u(\infty,t) = 0$ | 遠離板面的流體 | 板面運動尚未傳遞到此，維持靜止 | Dirichlet（截斷） |
| $u(y,0) = 0$ | 全場 $t=0$ 時刻 | 整個流體在平板起動前完全靜止 | 初始條件 |

> **no-slip 條件（無滑移）** 是黏性流體的基本假設：流體微元緊貼固體壁面時，其速度必須與壁面相同，不允許「滑動」。正是這個條件使板面動量得以透過黏滯力逐層傳遞至遠場。
...
**驗證方式**：對每個時刻快照，從數值解 $u_{\rm num}(y_i)$ 和解析解 $u_{\rm ana}(y_i) = U_0\,\mathrm{erfc}(y_i/2\sqrt{\nu t})$ 計算：

1. **速度場最大誤差**：$\max_i |u_{\rm num}(y_i) - u_{\rm ana}(y_i)|$（全場掃描）
2. **數值邊界層厚度 $\delta_{\rm num}$**：找 $u/U_0 = \mathrm{erfc}(2) \approx 0.00468$ 的 $y$ 值（線性插值），與解析式 $\delta_{\rm ana} = 4\sqrt{\nu t}$ 比較
...
**圖形說明：**
- **左圖（速度剖面）**：實線為 `py-pde` 數值解，虛線為解析解 $\mathrm{erfc}(y/2\sqrt{\nu t})$ 。在 $t = 0.00 \sim 0.50$ s 共 10 個快照下，兩條曲線幾乎完全重疊，肉眼無法區分，驗證擴散方程式數值精度。
- **右圖（邊界層成長）**：藍色實線為解析公式 $\delta_{\rm ana} = 4\sqrt{\nu t}$；紅色散點為從每個時刻數值解以閾值 $u/U_0 = \mathrm{erfc}(2)$ 插值得到的 $\delta_{\rm num}$。散點與曲線高度貼合，確認數值解正確追蹤了邊界層的 $\sqrt{t}$ 成長規律。

**邊界層厚度數值驗證（閾值 $u/U_0 = \mathrm{erfc}(2) \approx 0.00468$）：**

| 時刻 $t$ (s) | $\delta_{\rm ana}$ (cm) | $\delta_{\rm num}$ (cm) | 誤差 $\Delta\delta$ (mm) |
|:-----------:|:-------------------:|:-------------------:|:--------------------:|
| 0.05 | 0.8944 | 0.8961 | 0.017 |
| 0.10 | 1.2649 | 1.2661 | 0.012 |
| 0.15 | 1.5492 | 1.5501 | 0.009 |
| 0.20 | 1.7889 | 1.7902 | 0.013 |
| 0.25 | 2.0000 | 2.0008 | 0.008 |
| 0.30 | 2.1909 | 2.1920 | 0.011 |
| 0.35 | 2.3664 | 2.3668 | 0.004 |
| 0.40 | 2.5298 | 2.5307 | 0.009 |
| 0.45 | 2.6833 | 2.6842 | 0.009 |
| 0.50 | 2.8284 | 2.8292 | 0.007 |

> 邊界層厚度最大誤差 **< 0.02 mm**，相對誤差 **< 0.2%**。

**速度場逐點比較（解析解數值，數值解誤差 $< 10^{-4}$ m/s）：**

| $y$ (m) | $t=0.10$ s | $t=0.25$ s | $t=0.50$ s |
|:-------:|:----------:|:----------:|:----------:|
| 0.0050 | 0.26116 | 0.47729 | 0.61531 |
| 0.0101 | 0.02462 | 0.15522 | 0.31488 |
| 0.0201 | 0.00001 | 0.00447 | 0.04443 |
| 0.0402 | ≈ 0 | ≈ 0 | 0.00006 |
| 0.0598 | ≈ 0 | ≈ 0 | ≈ 0 |
| 0.0799 | ≈ 0 | ≈ 0 | ≈ 0 |

*表中數值單位為 m/s，即 $u_{\rm ana}(y,t)$；數值解與解析解之差在全場每個時刻均小於 $10^{-4}$ m/s（相對誤差 < 0.01%）。*

> **觀察重點**：在 $t=0.10$ s 時，速度只在 $y < 0.02$ m 範圍內有顯著值（ $2\sqrt{\nu t} = 0.00632$ m），動量尚未擴散至遠場；到 $t=0.50$ s 時邊界層已成長至 2.83 cm，速度在 $y \approx 0.02$ m 處仍有約 4.4% 的 $U_0$。
...
- **求解域（1D）**：圓管半徑 $r \in [0, R]$，利用**軸對稱性**將 2D 截面 $(r, \theta)$ 退化為 1D 問題（流場與 $\theta$ 無關）
- **充分展開條件（Fully Developed）**：管流入口後足夠遠處，速度剖面不再隨 $z$ 變化（ $\partial v_z/\partial z = 0$ ），整個截面速度分布固定為拋物線
- **壓力梯度 $dp/dz < 0$**：入口壓力大於出口（ $p_{\rm in} > p_{\rm out}$ ），壓差是驅動流動的唯一動力
- **徑向座標 $r$**：從管中心 $r=0$（速度最大）到管壁 $r=R$（速度為零），本問題只需對 $r$ 做 1D 求解
...
| 條件 | 幾何位置 | 物理意義 | BC 類型 |
|------|---------|---------|---------|
| $v_z(R) = 0$ | 管壁 $r=R$ | no-slip：流體黏附壁面，速度為零 | Dirichlet |
| $dv_z/dr\|_{r=0} = 0$ | 管中心 $r=0$ | 軸對稱：中心處速度梯度為零，速度取極大值 | Neumann |

> **為何中心用 Neumann 而非 Dirichlet？** 管中心的速度值 $v_z(0)$ 是未知量（正是求解目標），無法直接給定。但由軸對稱性可知，速度剖面關於 $r=0$ 對稱，其導數必為零。因此用 **Neumann BC $dv_z/dr=0$** 取代未知的 Dirichlet BC，加上管壁 Dirichlet 條件，即可唯一確定整個拋物線解。
...
| Reynolds 數 | 網格大小 | 時間步長 | 收斂迭代次數 | 最終殘差 $\lvert\Delta\psi\rvert_\infty$ |
|:-----------:|:-------:|:-------:|:----------:|:--------------------------------------:|
...
> **說明**：N=10 計算時間較 N=15 為長，係因首次運行時 Numba JIT 編譯需額外時間。N=15 → N=20 的時間差異極小（9.00 s → 9.58 s），在此小網格尺度下 $O(N^3)$ 的理論縮放尚不顯著；需更大網格（ $N > 50$ ）才能清楚觀察到此趨勢。
