...
| 類型 | 數學形式 | 化工熱傳對應情境 |
|------|---------|----------------|
| **Dirichlet (第一類)** | $T\big|_{\partial\Omega} = T_w$ | 壁面固定溫度（如沸騰/冷凝壁面） |
| **Neumann (第二類)** | $-k\,\partial T/\partial n\big|_{\partial\Omega} = q_w$ | 絕熱壁 ( $q_w=0$ ) 或固定熱通量（電熱膜加熱） |
| **Robin (第三類)** | $-k\,\partial T/\partial n\big|_{\partial\Omega} = h(T - T_\infty)$ | 對流散熱（Newton 冷卻定律，對流換熱係數 $h$ ） |
...
其中 $\sigma = 5.67 \times 10^{-8}$ $\mathrm{W/(m^2 \cdot K^4)}$ 為 Stefan-Boltzmann 常數， $\varepsilon$ 為表面輻射率； $T_K = T[^\circ C] + 273.15$ 為絕對溫度（輻射項 $T_K^4$ 必須使用絕對溫度 K，對流項 $h(T - T_\infty)$ 可用 °C 或 K）。
...
| 表面（Robin，對流+輻射） | $-k\,\partial T/\partial r = h(T_R - T_\infty) + \sigma\varepsilon(T_{R,K}^4 - T_{\infty,K}^4)$ |
...
其中 $T_K$ [K] 為絕對溫度（攝氏轉換： $T_K = T\,[{}^{\circ}\mathrm{C}] + 273.15$ ），而 $T_{\rm ref} = 300$ K（即 $27\,^\circ\mathrm{C}$）為參考溫度。
...
- $n = 1$ （深紅）： $k$ 從 $\approx 47$ $\mathrm{W/(m \cdot K)}$ （200°C）線性增至 $\approx 147$ $\mathrm{W/(m \cdot K)}$ （1200°C），倍數最大（ $\sim 3.1\times$ ）
- $n = -0.5$ （藍色）： $k$ 從 $\approx 24$ $\mathrm{W/(m \cdot K)}$ （200°C）降至 $\approx 14$ $\mathrm{W/(m \cdot K)}$ （1200°C），高溫端導熱最差
